...
(FastAPI service `mandare.api:app` + Typer CLI `rif`) backed by JSONL/JSON
...
- **Persistence goes through the helpers:** append-only logs via `JsonlStore`,
  ...
  file I/O elsewhere.
- **`MandareRuntime` is constructed fresh per process/test** (`MandareRuntime()`), not a
...
- **`src/mandare/api.py` is the source of truth for the API surface.** If a
  route changes, update `docs/API.md`, `README.md`, and
  `docs/MANDARE_MVP.md` to match.
...
- **Version bump checklist:** version derives from installed package metadata
  ...
  hardcoded version in `src/mandare/__init__.py`. `tests/test_version.py`
  catches drift.
