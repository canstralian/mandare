...
Mutable control-plane operations use `X-API-Key` and `RIF_CONTROL_PLANE_API_KEYS` and fail closed when no control-plane keys are configured. Inspection routes use `ReadPlaneAuth`; `POST /v1/runs` uses Supabase JWT identity. The authoritative unauthenticated inventory is `PUBLIC_ROUTES` (`tests/test_control_plane_auth.py:178`). Two of those entries carry a governance surface and are deliberately open **and** dry-run (`record=False`): `POST /v1/mcp/invoke` and `POST /v1/mcp/metasploit/evaluate` — making either recording, or adding another unauthenticated route that records, is a security-boundary change.
