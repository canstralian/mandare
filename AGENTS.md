...
- Wildcard rules are **not** inert. The shipped `deny_unknown_by_default` catch-all in `data/policies.json` and `DEFAULT_POLICIES` (`src/rif_runtime/configuration/policies.py:28`) is enforced, so with the shipped ruleset the effective fallback is **deny**. The engine's own built-in fallback is still `default.allow` (step 5 above) — deny-by-default is a property of the shipped `policies.json`, not of `PolicyEngine.evaluate` called with no catch-all. Deleting the catch-all therefore silently restores allow-by-default. See `tests/test_policy_store.py:105` and `:205`.
- Because the default denies what is not enumerated, the runtime's own first-party actions must be enumerated. `allow_run_create` backs `POST /v1/runs`, and the two ways of removing it now differ. Deleting it from a deployed `data/policies.json` is **repaired**: `PolicyStore._ensure_required_first_party_rules` (`configuration/policies.py:73`) reinserts any id in `REQUIRED_FIRST_PARTY_RULE_IDS` (`:57`) ahead of the catch-all on the next load (`tests/test_policy_store.py:301`). Deleting it from `DEFAULT_POLICIES` in source is **not** — that removes the rule and the repair's only source together, so `POST /v1/runs` 403s silently and the safety net goes with it. Treat a source deletion as blocking; a file deletion is self-healing.
...
`src/rif_runtime/api.py` is the source of truth for the HTTP surface and `src/rif_runtime/cli.py` for commands. The sync set differs by which surface changed:

- **A route change** must update `docs/API.md` (the full route index) and `docs/RIF_RUNTIME_MVP.md` (which enumerates the same route families), plus `README.md` if the route appears there.
- **A command change** must update `docs/cli-reference.md` (the canonical per-command reference) and the root `cli-reference.md`, which is a pointer that also inlines the command list and goes stale on its own. The CLI block in this file and in `CLAUDE.md` lists the same four commands and must be updated with them.

A diff that changes a surface without its documents is incomplete.
