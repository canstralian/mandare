...
class ManifestRegistry:
    ...
    def load(self, name: str, force_reload: bool = False) -> CapabilityManifest:
        ...
        error = ManifestLoadError(f"Manifest not found: {name} in {self.manifest_dir}")
    ...
    def get(
        self, name: str, default: CapabilityManifest | None = None
...
class ManifestValidator:
    ...
    def check_least_privilege(manifest: CapabilityManifest) -> list[str]:
        ...
        if manifest.filesystem_access.enabled:
            ...
            if not paths:
                warnings.append(
                    "filesystem_access.allowed_paths is empty (no filesystem access)"
                )
            for path in paths:
                if path == "/" or path == "/**":
                    warnings.append(
                    ...
                    warnings.append(
                        f"filesystem_access includes sensitive path: {path}"
                    )
        ...
        if manifest.network_access.enabled:
            ...
            if not domains:
                warnings.append(
                    "network_access.allowed_domains is empty (no network access)"
                )
            for domain in domains:
                if domain == "*" or domain == "*.com" or domain == "**":
                    warnings.append(f"network_access allows too broad domain: {domain}")
        ...
        if manifest.network_access.enabled:
            ...
            if "169.254.169.254" not in blocked:
                warnings.append(
                    "network_access does not block AWS metadata service "
                    "(169.254.169.254)"
                )
        ...
        if manifest.filesystem_access.access_level.value == "admin":
            warnings.append(
                "filesystem_access.access_level is ADMIN (use specific level)"
            )

        # Timeout too long
        if manifest.timeout_seconds > 300:
            warnings.append(
                f"timeout_seconds is very long: {manifest.timeout_seconds}s"
            )
    ...
    def check_evidence_completeness(manifest: CapabilityManifest) -> list[str]:
        ...
        if not manifest.evidence_requirements:
            warnings.append(
                "No evidence requirements defined (audit trail will be incomplete)"
            )

        immutable_count = sum(1 for e in manifest.evidence_requirements if e.immutable)
        if immutable_count == 0 and manifest.mutates_state:
            warnings.append(
                "No immutable evidence for mutating capability "
    ...
    @staticmethod
    def check_all(manifest: CapabilityManifest) -> dict[str, Any]:
        """Run all validation checks."""
        return {
            "risk_score": ManifestValidator.check_risk_score(manifest),
            "least_privilege_warnings": (
                ManifestValidator.check_least_privilege(manifest)
            ),
            "evidence_warnings": (
                ManifestValidator.check_evidence_completeness(manifest)
            ),
        }
...
if __name__ == "__main__":
    ...
    if manifest:
        ...
        lp_warnings = checks["least_privilege_warnings"]
        ev_warnings = checks["evidence_warnings"]
        assert isinstance(lp_warnings, list)
        assert isinstance(ev_warnings, list)
        print("\nSecurity checks:")
        print(f"  Risk score: {checks['risk_score']}/10")
        print(f"  Least privilege: {len(lp_warnings)} warnings")
        print(f"  Evidence: {len(ev_warnings)} warnings")
