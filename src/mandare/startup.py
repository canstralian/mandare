...
``validate_config`` runs at *import* of ``mandare.api``, which is the one
that actually fires in production. ``api.py`` builds a module-level
``MandareRuntime()``, and that constructor loads configuration -- so an invalid
...
def configuration_diagnostics() -> Iterator[None]:
    ...
    ``validate_config`` only covers settings *parsing*. ``MandareRuntime`` then
...
async def config_lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info(
        "Mandare runtime configuration loaded: %s",
