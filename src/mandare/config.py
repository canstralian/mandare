"""Runtime configuration contract for Mandare.
...
class RuntimeSection(BaseModel):
    ...
    def _blank_environment_is_unset(cls, value: object) -> object:
        ...
        (``or "production"``), while ``MandareRuntime._configured_environment``
...
def load_settings(
    ...
    """Load Mandare settings from TOML file + env-var overrides.
...
def load_config(path: str | Path | None = None) -> RuntimeConfig:
    ...
    Used by ``MandareRuntime.__init__``.  The path now defaults to
    ...
    if not path.is_file():
        ...
        # and MandareRuntime, which raises on an unknown name, would refuse to start
        # anywhere the file is not on disk. That is not hypothetical: the Vercel
        # entrypoint sets RIF_ENVIRONMENT=Mandare and runs from a CWD where
        # config/ may not be present, so cold start crashed with
        # "Mandare is not defined in environments.yaml (known: production)".
        #
