...
Mandare is a FastAPI service (`mandare.api:app`) plus a Typer CLI
...
`import mandare` then raised `ModuleNotFoundError`. Check first if
...
python -m uvicorn mandare.api:app --host 127.0.0.1 --port 8000 > /tmp/rif.log 2>&1 &
PID=$!
...
```text
{"status":"ok","environment":"Mandare","posture":"normal"}
{"current":"Mandare","environments":{...}}
{"agent":"agent:auditor",...}
...
Note: every run against a real `MandareRuntime()` (server or CLI) appends to
...
`src/mandare/execution/` (`ExecutionKernel`, `ExecutionManifest`,
`ExecutionResult`) plus `src/mandare/capabilities/` (`Capability`,
...
provider) registers into — see `.claude/skills/run-mandare/drive_capability_layer.py`.
...
- **`pip install -e .` silently no-ops on Python < 3.12.** `pyproject.toml`
  ...
  package — and a subsequent `import mandare` raises `ModuleNotFoundError`.
...
- **`rif serve`'s worker process evades `kill $!` and `pkill -f`.** Its
  ...
  `python -m uvicorn mandare.api:app --host 127.0.0.1 --port 8000`
